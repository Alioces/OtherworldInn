...
public class ItemHudOverlay {
    ...
    public record OverlayEntry(int priority, Predicate<Void> condition, BiConsumer<GuiGraphics, DeltaTracker> renderer) {}
     ...
     * @param renderer 渲染逻辑
     */
    public static void register(int priority, Predicate<Void> condition, BiConsumer<GuiGraphics, DeltaTracker> renderer) {
    ...
    public static void registerOverlays(RegisterGuiLayersEvent event) {
        event.registerAbove(VanillaGuiLayers.HOTBAR, ResourceLocation.fromNamespaceAndPath(OtherworldInn.MODID, "item_hud_overlay"), (guiGraphics, deltaTracker) -> {
    ...
    public static void renderMouseActions(GuiGraphics guiGraphics, MouseAction... actions) {
        ...
        for (MouseAction action : actions) {
            ...
            // 文本
            guiGraphics.drawString(font, action.text, currentX + iconSize + padding, startY + (iconSize - font.lineHeight) / 2 + 1, action.color, true);
